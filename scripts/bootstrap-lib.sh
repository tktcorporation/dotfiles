...
#
# なぜ取得経路が2つあるのか: setup.sh 実行時点ではリポジトリが未クローンなので
# リモートから curl するしかなく、run_onchange 側は chezmoi sourceDir が既にローカルに
# あるので直接読む。実行タイミングの制約による意図的な分離であり、1つの読み込み方法には
# 統合できない。リビジョンずれのリスクは setup.sh 側の関数存在チェックで検知する。
